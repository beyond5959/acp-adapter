# 基于 Codex App Server 的 Go 版 Codex ACP 适配器：详细可落地技术方案（全功能）
...
实现策略：
- 本适配器读取 `~/.config/codex-acp-go/config.toml`（或 JSON）中的 prompt profiles：  
...
4. **远程项目限制**：ChatGPT subscription 在 remote projects 不工作（保持与 codex-acp 行为一致，检测条件以客户端传入的 workspace 类型/路径为准）。  
...
ACP 的 content/tool 结构与 MCP 兼容，且 `codex-acp` 宣称支持“Client MCP servers”。  
...
- 记录每次 permission decision、每次文件写入/命令执行摘要（可选落盘到 `~/.cache/codex-acp-go/audit/`）。
...
codex-acp-go/
  cmd/
    codex-acp-go/              # main: ACP stdio server
  internal/
    acp/
...
### 8.1 启动 & 初始化
1. ACP client 启动 `codex-acp-go`（stdio）  
2. `codex-acp-go` spawn `codex app-server`（stdio JSONL）  
3. `codex-acp-go` → app-server: `initialize` + `initialized`   
4. ACP client → `codex-acp-go`: `initialize`（返回 capabilities & auth methods）  
...
- `codex-acp-go` 可执行文件（macOS/Linux/Windows）  
